...
import { ListedStocksTable } from "@/features/listed-stocks/components/listed-stocks-table";
...
describe("ListedStocksTable", () => {
  ...
  it("행과 종목명으로 연결 기업 상세를 열고 수정·삭제 클릭은 전파하지 않는다", () => {
    ...
    const row = screen.getByText(listedStock.code).closest("tr");
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: `${listedStock.name} 연결 기업 상세 보기`,
      }),
    );
    expect(onView).toHaveBeenCalledTimes(2);

    fireEvent.click(
      screen.getByRole("button", { name: `${listedStock.name} 수정` }),
    );
    expect(onEdit).toHaveBeenCalledWith(listedStock);
    expect(onView).toHaveBeenCalledTimes(2);

    fireEvent.click(
      screen.getByRole("button", { name: `${listedStock.name} 삭제` }),
    );
    expect(onDelete).toHaveBeenCalledWith(listedStock);
    expect(onView).toHaveBeenCalledTimes(2);
  });
  ...
  it("연결 기업을 찾지 못한 종목은 상세 동작을 제공하지 않는다", () => {
    ...
    fireEvent.click(screen.getByText(listedStock.code).closest("tr")!);
