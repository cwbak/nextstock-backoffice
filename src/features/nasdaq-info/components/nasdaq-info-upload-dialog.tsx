...
import { uploadNasdaqInfoCsv } from "@/data-access/queries/nasdaq-info/mutations";
import type { NasdaqInfoUploadResult } from "@/data-access/schemas/nasdaq-info";

const maxCsvFileSize = 4 * 1024 * 1024;

interface NasdaqInfoUploadDialogProps {
  onOpenChange: (open: boolean) => void;
  onUploaded: (result: NasdaqInfoUploadResult) => void;
  open: boolean;
}

export function NasdaqInfoUploadDialog({
  onOpenChange,
  onUploaded,
  open,
}: NasdaqInfoUploadDialogProps) {
  const inputId = useId();
  const queryClient = useQueryClient();
  const [file, setFile] = useState<File | null>(null);
  const [fileError, setFileError] = useState<string | null>(null);
  const mutation = useMutation({
    mutationFn: uploadNasdaqInfoCsv,
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: nasdaqInfoKeys.all });
      setFile(null);
      setFileError(null);
      onUploaded(result);
      onOpenChange(false);
