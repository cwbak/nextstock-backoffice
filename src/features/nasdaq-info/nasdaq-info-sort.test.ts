...
function createItem(symbol: string, marketCap: string | null): NasdaqInfo {
  return {
    symbol,
    name: symbol,
    marketCap,
...
describe("sortNasdaqInfoByMarketCap", () => {
  it("큰 시가총액의 정밀도를 유지해 오름차순 정렬한다", () => {
    expect(
      sortNasdaqInfoByMarketCap(items, "asc").map((item) => item.symbol),
    ).toEqual(["A", "B", "C"]);
  });

  it("내림차순에서도 null 시가총액을 마지막에 둔다", () => {
    expect(
      sortNasdaqInfoByMarketCap(items, "desc").map((item) => item.symbol),
    ).toEqual(["B", "A", "C"]);
  });
});
