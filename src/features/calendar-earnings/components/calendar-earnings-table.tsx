...
export interface CalendarEarningsTableItem {
...
interface CalendarEarningsTableProps {
  ...
  items: ReadonlyArray<CalendarEarningsTableItem>;
}

export function CalendarEarningsTable({
  classificationLabel,
  items,
}: CalendarEarningsTableProps) {
  return (
    <Table className="min-w-[76rem] table-fixed">
      ...
      <TableHeader>
        <TableRow className="bg-muted/35 hover:bg-muted/35">
          ...
          <TableHead>{classificationLabel}</TableHead>
      ...
      <TableBody>
        {items.map((item) => (
          <TableRow key={`${item.identifier}:${item.reportDate}`}>
