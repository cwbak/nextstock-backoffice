...
interface NasdaqStockUploadDialogProps {
  onOpenChange: (open: boolean) => void;
  onUploaded: (result: NasdaqStockUploadResult) => void;
  open: boolean;
}

export function NasdaqStockUploadDialog({
  onOpenChange,
  onUploaded,
  open,
}: NasdaqStockUploadDialogProps) {
  const inputId = useId();
  const queryClient = useQueryClient();
  const [file, setFile] = useState<File | null>(null);
  const [fileError, setFileError] = useState<string | null>(null);
  const mutation = useMutation({
    mutationFn: uploadNasdaqStockCsv,
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: nasdaqStockKeys.all });
      setFile(null);
      setFileError(null);
      onUploaded(result);
      onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={changeOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle>NASDAQ 종목 정보 CSV 업로드</DialogTitle>
          <DialogDescription>
            Nasdaq Stock Screener에서 내려받은 CSV를 Symbol 기준으로 추가하거나
