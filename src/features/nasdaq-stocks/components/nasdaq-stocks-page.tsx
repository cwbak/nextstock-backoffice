...
import type { NasdaqStockUploadResult } from "@/data-access/schemas/nasdaq-stock";
import { NasdaqStockEmptyState } from "@/features/nasdaq-stocks/components/nasdaq-stock-empty-state";
import { NasdaqStocksTable } from "@/features/nasdaq-stocks/components/nasdaq-stocks-table";
import { NasdaqStockUploadDialog } from "@/features/nasdaq-stocks/components/nasdaq-stock-upload-dialog";
import { NasdaqStockUploadSummary } from "@/features/nasdaq-stocks/components/nasdaq-stock-upload-summary";
import {
  filterByMarketCap,
...
export function NasdaqStocksPage() {
  const nasdaqStocksQuery = useSuspenseQuery(nasdaqStocksQueryOptions);
  const {
  ...
  }, [filterQuery, marketCapFilter, nasdaqStocksQuery.data, sp500Filter]);
  const sortedItems = useMemo(
    () =>
      tableState.sortBy === null
        ? filteredItems
        : sortNasdaqStocksByMarketCap(filteredItems, tableState.sortDirection),
      ...
      <section className="flex flex-col gap-6">
        <DataPageHeader
          actions={
            <>
              <Button
                disabled={nasdaqStocksQuery.isFetching}
                type="button"
                variant="outline"
                onClick={() => void nasdaqStocksQuery.refetch()}
              >
                {nasdaqStocksQuery.isFetching ? (
          ...
          eyebrow="NASDAQ stocks"
          recordCount={nasdaqStocksQuery.data.length}
          title="나스닥 정보"
        />
        {uploadResult ? (
          <NasdaqStockUploadSummary
            result={uploadResult}
        ...
        <DataTableCard
          ...
          title="NASDAQ 종목 정보"
        >
          {nasdaqStocksQuery.data.length === 0 ? (
            <NasdaqStockEmptyState onUpload={() => setUploadOpen(true)} />
