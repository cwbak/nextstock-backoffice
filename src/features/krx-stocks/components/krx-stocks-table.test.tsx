...
import { KrxStocksTable } from "@/features/krx-stocks/components/krx-stocks-table";
...
describe("KrxStocksTable", () => {
  ...
  it("행과 종목명으로 연결 기업 상세를 열고 수정·삭제 클릭은 전파하지 않는다", () => {
    ...
    const row = screen.getByText(krxStock.code).closest("tr");
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: `${krxStock.name} 연결 기업 상세 보기`,
      }),
    );
    expect(onView).toHaveBeenCalledTimes(2);

    fireEvent.click(
      screen.getByRole("button", { name: `${krxStock.name} 수정` }),
    );
    expect(onEdit).toHaveBeenCalledWith(krxStock);
    expect(onView).toHaveBeenCalledTimes(2);

    fireEvent.click(
      screen.getByRole("button", { name: `${krxStock.name} 삭제` }),
    );
    expect(onDelete).toHaveBeenCalledWith(krxStock);
    expect(onView).toHaveBeenCalledTimes(2);
  });
  ...
  it("연결 기업을 찾지 못한 종목은 상세 동작을 제공하지 않는다", () => {
    ...
    fireEvent.click(screen.getByText(krxStock.code).closest("tr")!);
