...
import { createKrxStock } from "@/data-access/queries/krx-stocks/mutations";
import {
  krxStockCreatePayloadSchema,
  type KrxStockCreatePayload,
} from "@/data-access/schemas/krx-stock";

interface KrxStockCreateDialogProps {
  onOpenChange: (open: boolean) => void;
  open: boolean;
}

const createFieldNames = new Set<keyof KrxStockCreatePayload>([
  "corporationCode",
  "stockCode",
]);
...
function isCreateFieldName(
  value: PropertyKey,
): value is keyof KrxStockCreatePayload {
  return createFieldNames.has(value as keyof KrxStockCreatePayload);
}

interface KrxStockCreateFormProps {
  onCancel: () => void;
  onSaved: () => void;
}

function KrxStockCreateForm({ onCancel, onSaved }: KrxStockCreateFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: createKrxStock,
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: krxStockKeys.all }),
  ...
  const submitForm = handleSubmit((values) => {
    clearErrors();
    const result = krxStockCreatePayloadSchema.safeParse(values);
    ...
    >
      <FieldGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Field data-invalid={Boolean(errors.stockCode)}>
          <FieldLabel htmlFor="create-krx-stock-code">종목 코드</FieldLabel>
          <Input
            {...stockCodeRegistration}
            aria-invalid={Boolean(errors.stockCode)}
            id="create-krx-stock-code"
            maxLength={6}
            placeholder="005930"
            onChange={(event) => {
...
export function KrxStockCreateDialog({
  onOpenChange,
  open,
}: KrxStockCreateDialogProps) {
  return (
