...
export function CorporationInvestmentsBulkCreateSummary({
  ...
  return (
    <Alert>
      ...
      <AlertTitle>
        {hasFailures
          ? "일부 법인을 제외하고 출자현황 동기화를 완료했습니다"
          : "전체 법인 출자현황 동기화를 완료했습니다"}
      </AlertTitle>
      <AlertDescription className="flex flex-col gap-1">
        <span>
          전체 {result.corporationCount.toLocaleString("ko-KR")}개 법인 중 성공{" "}
          {result.processedCount.toLocaleString("ko-KR")}개, 실패{" "}
          {result.failedCount.toLocaleString("ko-KR")}개
        </span>
        <span>
          DART 조회 {result.fetchedCount.toLocaleString("ko-KR")}건 · 반영{" "}
          {result.upsertedCount.toLocaleString("ko-KR")}건
        </span>
      </AlertDescription>
      <AlertAction>
        <Button
          aria-label="전체 출자현황 동기화 결과 닫기"
          size="icon-sm"
          type="button"
          variant="ghost"
          onClick={onClose}
